...
`AUTOMATIC LABELS ARE AS EFFECTIVE AS MANUAL LABELS IN BIOMEDICAL IMAGES CLASSIFICATION WITH DEEP LEARNING <https://arxiv.org/pdf/2406.14351>`_
    Lack of labels is a big issue in training deep learning models.  This paper explores whether you can use automatic labels and find that you can, as long as the percentage of noisy labels is less than 10%.  Seems morally similar to the way Depth Anything V2 was trained and is something we should look into.
...
`Using an LLM to Compete in an AGI Banchmark<https://redwoodresearch.substack.com/p/getting-50-sota-on-arc-agi-with-gpt>`_
    There is something interesting here. LLMs can kind of compete in AGI benchmarks that require a lot of reasoning, but they involve so much prompt enginnering that maybe you would have been better writting code instead of orchestration code.

`Getting Structured Output from LLMs <https://www.boundaryml.com/blog/structured-output-from-llms>`_
    To us, LLMs are primarily tools to turn text in to structured information. What python tools are there to get you there? Here is a comparison.

`Logit Prisms <https://neuralblog.github.io/logit-prisms/>`_
    Comes up with a method to read the tea leaves of logits coming out of an LLM. I'm not very convinced, but this has some similarities to what Miles is sometimes interested in.
...
`Refusal in Language Models Is Mediated by a Single Direction <https://arxiv.org/abs/2406.11717>`_
    The RLHF that tells the LLM to be nice and refuse to answer sometimes, apparently is findiable in the gradients during runtime. If you just remove that dimension with a hack, you get an uncensored LLM from your censored weights.
